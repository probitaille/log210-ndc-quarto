...
À noter que le livre de @craig_uml_2005 présente quelques RDCU qui sont des *diagrammes de communication*. 
...
Les RDCU décrivent comment un système réalise les fonctionnalités spécifiées dans un cas d'utilisation.

Cela est généralement fait en reliant le cas d'utilisation à d'autres informations spécifiées dans le MDD, le DSS et les contrats d'opération, en utilisant des diagrammes de séquence UML afin de détailler les interactions entre les différents objets ou composants du système qui accomplissent l'objectif de l'acteur. Une RDCU sert à esquisser une solution (qui n'est pas encore codée) afin de rendre plus explicite l'activité impliquant des choix de conception. Si vous n'avez pas bien compris les autres éléments (MDD, DSS, etc.), il vous sera difficile de réussir les RDCU.
...
## Notation UML des diagrammes de séquence {#sec-notation-sequence}

Les diagrammes de séquence UML utilisent une notation graphique standardisée pour représenter les interactions entre objets au fil du temps. Cette section présente les éléments essentiels de cette notation.

### Participants (objets et instances)

Les objets sont représentés par des rectangles en haut du diagramme. Chaque participant possède :

- **Un nom** : peut être un nom d'instance (`nomInstance`), un nom de classe (`:MaClasse`), ou les deux (`nomInstance:MaClasse`)
- **Une ligne de vie** : ligne verticale pointillée descendant du rectangle, représentant l'existence de l'objet dans le temps

::: {.callout-tip}
## Convention de notation
- `:NomClasse` indique une instance anonyme de la classe
- `nomInstance:NomClasse` identifie une instance spécifique
- Le deux-points est toujours présent entre le nom d'instance et le nom de classe
- Les noms soulignés représentent des instances concrètes d'objets, c'est-à-dire des objets qui existent réellement en mémoire lors de l'exécution. Dans un diagramme de séquence, vous travaillez presque toujours avec des instances (donc soulignées), car les séquences montrent des interactions entre objets concrets durant l'exécution.
:::

```{.plantuml genurl="true" #fig-RDCU_Identification_Participants caption="Identifications des participants dans un diagramme de séquence UML." }
@startuml
!include ecriture.pumlinclude
hide footbox
participant "__patrice:Enseignant__" as Patrice

note over of Patrice
Instance nommée
end note

participant "__:Etudiant__" as Etudiant

note over of Etudiant
Instance anonyme
end note
...
d'une classe sans instance
end note

@enduml
```

### Boîtes d'activation (barres d'activation)

Les boîtes d'activation sont des rectangles étroits superposés sur la ligne de vie d'un objet. Elles indiquent :

- **La période d'activité** : quand l'objet exécute une opération ou attend une réponse
- **Les appels imbriqués** : une boîte peut contenir d'autres boîtes, montrant la profondeur de la pile d'appels
- **La durée d'exécution** : la hauteur de la boîte représente le temps d'exécution

Ces boîtes peuvent être omises pour simplifier le dessin à la main, mais elles sont utiles et recommandées pour bien comprendre la séquence d'exécution et visualiser la complexité des interactions.

```{.plantuml genurl="true" #fig-RDCU_Boite_Activation caption="Exemple de diagramme de séquence avec une boite d'activation." }
@startuml
hide footbox
...
A -> A: unAppelInterne():Bool
activate A #DarkSalmon

A -> B: unAutreAppel():Bool
activate B
...
Les messages sont représentés par des flèches entre les lignes de vie des participants. Chaque type de flèche a une signification précise :

#### Messages synchrones (appels de méthode)

- **Notation** : Flèche pleine (→)
- **Signification** : Appel de méthode synchrone où l'émetteur attend la fin de l'exécution
- **Syntaxe** : `nomMethode(parametre1:type, parametre2:type):TypeRetour`
- **Exemple** : `calculerTotal(item1:float, item2:float):float`

#### Messages de retour

- **Notation** : Flèche pointillée (⇢)
- **Signification** : Valeur retournée à la fin de l'exécution d'une méthode
- **Syntaxe** : `nomValeur:TypeRetour`

::: {.callout-note}
## Affectation vs flèche de retour
La flèche de retour pointillée est la notation UML standard et est utilisée pour plus de clarté dans les diagrammes formels. Cette façon de faire met en évidence le moment précis du retour, surtout si d'autres opérations se produisent entre l'appel et le retour.

Il est aussi possible d'utiliser une forme plus concise par affectation (`variable = methode()`), ce qui ressemble plus au code. C'est utile quand le retour est immédiat ou quand il n'est pas utile de mettre l'accent sur le moment exact du retour.
:::

#### Messages de création d'objets

- **Notation** : Flèche pleine pointant vers le rectangle de l'objet créé (pas vers sa ligne de vie)
- **Stéréotype** : Peut inclure `<<create>>` ou le constructeur de l'objet `new Object()`
- **Placement** : Le rectangle de l'objet et sa ligne de vie de l'objet créé commence au point de création sur l'axe vertical

::: {.callout-note}
## Flèche pleine vs flèche pointillée

Des versions de UML 1.x (années 1990)** utilisaient une flèche pointillée avec le stéréotype `<<create>>` pour la création d'objets.

Depuis les version de UML 2.x (année 2005+), la norme a été révisée pour utiliser une flèche pleine, alignée avec la sémantique d'appel synchrone
:::

```{.plantuml genurl="true" #fig-RDCU_Creation caption="Exemple de diagramme de séquence avec la création d'une instance Joueur." }
@startuml
hide footbox
autoactivate on

participant "__:Jeu__" as Jeu

-> Jeu : démarrer()
Jeu -> "__j1:Joueur__" ** : <<create>>

@enduml
```

#### Messages à soi-même (auto-appel)

- **Notation** : Flèche qui revient vers la même ligne de vie
- **Signification** : L'objet s'envoie un message à lui-même (appel de méthode privée)
- **Boîte d'activation** : Crée une nouvelle boîte d'activation imbriquée

### Notes et commentaires

Les notes (rectangles avec coin replié) permettent d'ajouter :

- **Justifications GRASP** : Documenter les principes de conception appliqués
- **Contraintes** : Préciser des conditions ou limitations
- **Explications** : Clarifier des choix de conception complexes

### Ressources complémentaires
Pour approfondir la notation UML des diagrammes de séquence, consultez :

- [Spécification UML 2.5.1 officielle](https://www.omg.org/spec/UML/)
- [Tutoriel PlantUML pour les diagrammes de séquence](https://plantuml.com/sequence-diagram)
:::


## Processus de création d'une RDCU
...
participant "__:Plateau__" as p
participant "__:Case__" as c
loop
create c
p ->> c : create(nom)
end loop
...
scale 1.3
skinparam sequenceMessageAlign left
participant "__a:A__" as A
participant "__b:B__" as B
-> A : ...
activate A
A -> B : unMessage()
activate B
...
participant "__:Registre__" as R
participant "__:Magasin__" as M
participant "__v:Vente__" as v
-> R : saisirClientPourRemise(idClient:String)
activate R
R -> M : c = getClient(idClient:String)
activate M
...
skinparam sequenceMessageAlign center
participant "__:Plateau__" as P
participant "__cMap\n:Map<String, Case>__" as m
-> P : s = getCase(nom:String)
activate P
...
participant "__:JeuRisk__" as jr
participant "__:Dé__" as d
participant "__:PlateauRisk__" as pr
participant "__:Continent__" as c
participant "__p:Pays__" as p
participant "__pMap\n:Map<String,\nPays>__" as m
create jr
->> jr : <<create>>
...
scale 1.0
!include normal.pumlinclude
participant "__:A__" as B
participant "__:B__" as A
[-> B : execute(3)
activate B
...
1. Écrivez le code pour la figure suivante.
    ...
    !include normal.pumlinclude
    scale 1.0
    participant "__:Bernard__" as Bob
    participant "__:Alice__" as Alice
    participant "__:Autre__" as Other
    [-> Bob : init
    activate Bob
    Bob -> Alice : allô(12)
    activate Alice
